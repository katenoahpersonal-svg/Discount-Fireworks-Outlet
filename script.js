...
const classes=["build-panel","locations-panel","hero-panel","directions-panel","contact-panel"];
let active=2;
...
function render(){
  panels.forEach((panel,i)=>{
    ...
    panel.classList.add(classes[mapped]);
  });
}
...
document.querySelector(".carousel").addEventListener("pointerup",e=>{
  ...
  if(Math.abs(diff)>40){active=diff<0?(active+1)%panels.length:(active-1+panels.length)%panels.length;render();}
  startX=null;
});
render();
